...
export const setupMessageHandler = (io: Server, socket: AuthenticatedSocket): void => {
  ...
  socket.on('message:send', async (payload: SendMessagePayload) => {
    try {
      const { conversationId, content, type, tempId, fileUrl, fileName, fileSize, senderEncryptedKey, recipientEncryptedKey, iv, replyToId } = payload;
  ...
  // Delete message
  socket.on('message:delete', async ({ messageId, conversationId }: { messageId: string; conversationId: string }) => {
    try {
      const message = await messageService.deleteMessage(conversationId, socket.userId, messageId);

      // Broadcast message deletion to the entire conversation room
      const room = getConversationRoom(conversationId);
      ...
      logger.debug(`Message ${messageId} deleted in ${conversationId} by ${socket.userId}`);
    } catch (error) {
