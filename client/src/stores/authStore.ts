...
export const useAuthStore = create<AuthState>((set, get) => ({
  ...
  socialLogin: async (
    ...
    try {
      ...
      const requestPayload: any = { email, username };

      if (keysSetup) {
        requestPayload.publicKey = keysSetup.publicKey;
        requestPayload.encryptedPrivateKey = keysSetup.encryptedPrivateKey;
        requestPayload.keySalt = keysSetup.keySalt;
        requestPayload.backupPin = pin;
      }

      const { data } = await api.post<AuthResponse>('/auth/social-login', requestPayload);

      // If user has keys and PIN is provided, try to decrypt/save
      if (data.user.encryptedPrivateKey && data.user.keySalt && pin) {
        try {
          const masterKey = await deriveMasterKey(pin, data.user.keySalt);
          const { privateKeyJwk } = await decryptPrivateKey(data.user.encryptedPrivateKey, masterKey);
          localStorage.setItem('e2e_private_key', JSON.stringify(privateKeyJwk));
        } catch (err) {
          console.error("[Auth] Failed to decrypt private key with social PIN", err);
      ...
      set({
        isLoading: false,
        error: error.message || error.response?.data?.error || 'Social login failed.',
      });
      throw error;
    }
  },
