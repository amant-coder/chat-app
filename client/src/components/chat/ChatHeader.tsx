...
export default function ChatHeader() {
  ...
  return (
    <div className="flex-shrink-0">
      {/* Main header row */}
      <div className="h-14 sm:h-16 flex items-center justify-between px-2 sm:px-4 border-b border-(--border) bg-(--bg-secondary)/80 backdrop-blur-xl">
        <div className="flex items-center gap-2 sm:gap-3 min-w-0 flex-1">
          ...
          <div className="flex items-center md:hidden flex-shrink-0">
            <button
              ...
              className="p-2 rounded-xl hover:bg-(--bg-hover) transition-colors text-(--text-secondary) active:scale-95"
              aria-label="Back"
            >
          ...
          <div className="relative flex-shrink-0">
            <img
              src={headerAvatar}
              alt=""
              className="w-9 h-9 sm:w-10 sm:h-10 rounded-full bg-(--bg-hover) object-cover"
            />
          ...
          <div className="min-w-0">
            <h3 className="text-sm font-semibold text-(--text-primary) truncate">{headerName || 'Unknown'}</h3>
        ...
        <div className="flex items-center gap-0.5 sm:gap-1 flex-shrink-0">
          {isGroup && activeConversation.admins?.includes(currentUser?._id || '') && (
            <button
              ...
              className="p-2 rounded-xl transition-colors hover:bg-(--bg-hover) text-(--text-secondary) active:scale-95"
          ...
          <button
            ...
            className={`p-2 rounded-xl transition-colors active:scale-95 ${isSearchOpen ? 'bg-(--accent)/10 text-(--accent)' : 'hover:bg-(--bg-hover) text-(--text-secondary)'}`}
      ...
      {/* Search panel */}
      {isSearchOpen && (
        <div className="border-b border-(--border) bg-(--bg-secondary) px-3 sm:px-4 py-2.5 sm:py-3 space-y-2 animate-[slideDown_0.2s_ease-out]">
          <div className="flex items-center gap-2 px-3 py-2 bg-(--bg-tertiary) border border-(--border) rounded-xl">
            ...
            <button onClick={closeSearch} className="text-(--text-muted) hover:text-(--text-primary) flex-shrink-0 p-1">
            ...
            <div className="max-h-48 overflow-y-auto rounded-xl border border-(--border) divide-y divide-(--border)">
              {searchResults.map((msg) => (
                <div
                  ...
                  className="px-3 py-2 hover:bg-(--bg-hover) transition-colors cursor-pointer active:bg-(--bg-active)"
                >
